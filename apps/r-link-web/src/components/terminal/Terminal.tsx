...
export const WebTerminal: React.FC<TerminalProps> = ({
  ...
  const connect = useCallback(async () => {
    if (socketAttemptRef.current || wsRef.current?.readyState === WebSocket.OPEN || wsRef.current?.readyState === WebSocket.CONNECTING) {
      return;
    }

    const attempt = new AbortController();
    socketAttemptRef.current = attempt;
    ...
    try {
      ...
      const ws = await openSshSocket(url, attempt.signal);
      if (attempt.signal.aborted) { ws.close(); return; }
      wsRef.current = ws;
