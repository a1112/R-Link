...
export const ConsoleView: React.FC = () => {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const response = await authenticatedFetch(`${API_BASE}/api/console/status`);
      if (response.ok) {
  ...
  const startConsole = async () => {
    setIsLoading(true);
    try {
      const response = await authenticatedFetch(`${API_BASE}/api/console/start`, {
        method: "POST",
      });
  ...
  const stopConsole = async () => {
    setIsLoading(true);
    try {
      const response = await authenticatedFetch(`${API_BASE}/api/console/stop`, {
        method: "POST",
      });
      const result: ApiResponse = await response.json();
  ...
  const restartConsole = async () => {
    setIsLoading(true);
    try {
      const response = await authenticatedFetch(`${API_BASE}/api/console/restart`, {
        method: "POST",
      });
      const result: ApiResponse = await response.json();
  ...
  const updateConfig = async () => {
    setIsLoading(true);
    try {
      const response = await authenticatedFetch(`${API_BASE}/api/console/config`, {
        method: "POST",
